...
void predict(float dt, Vector3 gyro_raw) {
    ...
    // 1. Calculate True Gyro          w_true = w_raw *pi/180 - bias   
...
void Task_ROLL_PITCH(void *pvParameters) {
    ...
    while(1) {
        ...
        if (xSemaphoreTake(ekfMutex, portMAX_DELAY) == pdTRUE) {
            ...
            // 3. Extract Euler Angles for Control
            float q0 = x(0,0), q1 = x(1,0), q2 = x(2,0), q3 = x(3,0);
...
Serial.begin(115200);
    ...
    xTaskCreatePinnedToCore(Task_ROLL_PITCH, "Pilot", 8192, NULL, 2, &TaskPilot_Handle, 1);
    xTaskCreatePinnedToCore(Task_YAW, "Nav", 4096, NULL, 1, &TaskNav_Handle, 0);
}
